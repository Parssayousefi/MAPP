...
# Prepare Data (assuming predictors are in columns other than 'wellbeing' and 'subjectID')
predictors <- train_data[,-c(which(colnames(train_data) == "wellbeing"), 
                             which(colnames(train_data) == "subjectID"))]
outcome <- train_data$wellbeing

# Fit Lasso model
lasso_model <- cv.glmnet(x = as.matrix(predictors), y = outcome, alpha = 1, family = "gaussian")

# Extract selected variables
best_lambda <- lasso_model$lambda.min  
selected_vars <- names(predictors)[coef(lasso_model, s = best_lambda) != 0]

# (Optional) Create a dataframe summarizing the results. 

# Step 2: Correlational Analysis
correlations <- cor(train_data[,-which(colnames(train_data) == "subjectID")])  # Calculate correlations
wellbeing_corrs <- correlations[,"wellbeing"]  # Extract well-being correlations
ranked_corrs <- sort(wellbeing_corrs, decreasing = TRUE) 

# Step 3: Fit a 1-Factor Model for Well-being Questionnaires 
# (Assuming well-being items are in columns named WB1, WB2, ...)
wb_model_string <- paste("WB =~ WB1 + WB2 + ... + WBn", collapse = " ")
wb_fit <- cfa(wb_model_string, data = train_data, estimator = "ML")
summary(wb_fit, fit.measures = TRUE, standardized = TRUE)

# Step 4: Extract Factor Scores
factor_scores <- lavPredict(wb_fit)  # Assumes good model fit 
